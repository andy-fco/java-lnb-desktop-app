...
import GUI.MenuDeJuegos;

public interface Juegos {
	
	default public void jugarAhorcado(Aficionado aficionado) {
	    String[][] palabrasPorNivel = {
	    		{"ferro", "doble", "tapon", "pivot", "alero", "salto", "falta", "poste", "obras", "final"}, // 5 letras
	        { "volcada", "triples", "bandeja", "arbitro", "campeon", "playoff", "defensa", "regatas", "silbato", "tirador"}, // 7 letras
	        { "picknroll", "zapatilla", "flotadora", "crossover", "instituto", "argentino", "fanaticos", "chicharra", "suplentes", "titulares" } // 9 letras
	    };
	    ...
	    while (true) {
	        ...
	        if (nivel == 3 || nivel == JOptionPane.CLOSED_OPTION) {
	        	MenuDeJuegos frame = new MenuDeJuegos(aficionado);
            	frame.run(aficionado);
	            return;
	        }
	        ...
	        while (palabrasJugadas < 10) {
	            ...
	            if (aciertos == longitud) {
	                ...
	                mensaje = "¡¡Felicitaciones!! Adivinaste la palabra: " + palabra + "\n¡Sumás "+longitud+" puntos!";
	                try {
	                	aficionado.sumarPuntos(longitud);
	                    
	                } catch (Exception e) {
	                    e.printStackTrace();
	                    JOptionPane.showMessageDialog(null, "Lo sentimos, hubo un error al sumar los puntos.", "Aviso", JOptionPane.WARNING_MESSAGE);
	                }
	            } else {
