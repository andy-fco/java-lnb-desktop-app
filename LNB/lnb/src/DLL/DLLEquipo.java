...
public class DLLEquipo {
	...
	public static boolean editarEquipo(Equipo equipo, int id) {
	    String sql = "UPDATE equipos SET ciudad = ?, estadio = ?, fecha_de_fundacion = ?, temporadas_en_liga = ?, campeonatos = ?, nombre = ? WHERE id = ?";

	    try (PreparedStatement stmt = con.prepareStatement(sql)) {
	        stmt.setString(1, equipo.getCiudad());
	        stmt.setString(2, equipo.getEstadio());
	        stmt.setDate(3, Date.valueOf(equipo.getFechaDeFundacion()) );
	        stmt.setInt(4, equipo.getTemporadasEnLiga());
	        stmt.setInt(5, equipo.getCampeonatos());
	        stmt.setString(6, equipo.getNombre()); 
	        stmt.setInt(7, id);
