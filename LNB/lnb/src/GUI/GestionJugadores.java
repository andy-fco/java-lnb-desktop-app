...
public class GestionJugadores extends JFrame {
	...
	public GestionJugadores(Admin admin) {
			...
			if (!e.getValueIsAdjusting()) {
				...
				if (row != -1) {
					...
					jugadorSeleccionado = new Jugador((String) model.getValueAt(row, 0),
							...
							(Integer) model.getValueAt(row, 3), (String) model.getValueAt(row, 4), equipo,
							(Integer) model.getValueAt(row, 6)

					);
					lblSeleccionado.setText("Seleccionado: " + jugadorSeleccionado.getNombre() + " "
							+ jugadorSeleccionado.getApellido() + " | " + jugadorSeleccionado.getEquipo().getNombre());
		...
		btnEditar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (jugadorSeleccionado != null) {
					 ...
					 EditarJugador frame = new EditarJugador(admin, jugadorSeleccionado); 
					 frame.run(admin, jugadorSeleccionado); 
					 dispose();
		...
		JComboBox<String> comboBoxPosicion = new JComboBox<>();
		comboBoxPosicion.setModel(
				new DefaultComboBoxModel(new String[] { "", "Base", "Escolta", "Alero", "Ala-Pivot", "Pivot" }));
	...
	public void cargarTabla() {
		...
		for (Jugador j : jugadores) {
			model.addRow(new Object[] { j.getNombre(), j.getApellido(), j.getNacionalidad(), j.getCamiseta(),
					j.getPosicion(), j.getEquipo().getNombre(), j.getMedia() });
		}
	}
	...
	public void buscarPorPosicion(String texto) {
		LinkedList<Jugador> jugadores = DLLJugador.mostrarJugadoresPorPosicion(texto);
		jugadores.sort(Comparator.comparing(Jugador::getMedia).reversed());
