...
public class MejorarMyPlayer extends JFrame {
	 ...
	 * Create the frame.
	 */
	public MejorarMyPlayer(Aficionado user, Jugador myPlayer) {
		...
		lblPosicion.setBounds(127, 124, 241, 32);
		contentPane.add(lblPosicion);
		
		JLabel lblNacionalidad = new JLabel("<html>Acá vas a poder mejorar a tu jugador usando los puntos que llevás acumulados. Podés mejorar sus habilidades en cancha y su media. Por cada punto que tu jugador mejore en cualquier área, vas a tener que pagar con 10 de tus puntos. Mejorar habilidades o media tiene exactamente el mismo costo. Pensá con cuidado cómo querés que crezca tu jugador.\r\n</html>");
		lblNacionalidad.setFont(new Font("Tahoma", Font.PLAIN, 18));
		lblNacionalidad.setBounds(360, 146, 304, 280);
		...
		spinnerTiro.setModel(new SpinnerNumberModel(myPlayer.getTiro(), myPlayer.getTiro(), 99, 1));
		...
		spinnerVelocidad.setModel(new SpinnerNumberModel(myPlayer.getVelocidad(), myPlayer.getVelocidad(), 99, 1));
		...
		spinnerDribling.setModel(new SpinnerNumberModel(myPlayer.getDribling(), myPlayer.getDribling(), 99, 1));
		...
		spinnerDefensa.setModel(new SpinnerNumberModel(myPlayer.getDefensa(), myPlayer.getDefensa(), 99, 1));
		...
		spinnerPase.setModel(new SpinnerNumberModel(myPlayer.getPase(), myPlayer.getPase(), 99, 1));
		...
		spinnerSalto.setModel(new SpinnerNumberModel(myPlayer.getSalto(), myPlayer.getSalto(), 99, 1));
		...
		spinnerMedia.setModel(new SpinnerNumberModel(myPlayer.getMedia(), myPlayer.getMedia(), 99, 1));
		...
		btnMejorar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int total = myPlayer.getMedia()+
						myPlayer.getTiro()+
						myPlayer.getDribling()+
						myPlayer.getPase()+
						myPlayer.getVelocidad()+
						myPlayer.getDefensa()+
						myPlayer.getSalto();
				int mejoras = (Integer) spinnerMedia.getValue() +
					    (Integer) spinnerTiro.getValue() +
					    (Integer) spinnerDribling.getValue() +
					    (Integer) spinnerPase.getValue() +
					    (Integer) spinnerVelocidad.getValue() +
					    (Integer) spinnerDefensa.getValue() +
					    (Integer) spinnerSalto.getValue();
				
				int dif = mejoras - total;
				
				if (dif*10>puntos) {
					JOptionPane.showMessageDialog(null, "Parece que no te alcanzan los puntos para hacer esas mejoras."
							+ "\nRevisá tus estadísticas y volvé a intentarlo");
				} else {
					if (DLLJugador.mejorarJugador((Integer) spinnerMedia.getValue(),
							(Integer) spinnerTiro.getValue(),
							(Integer) spinnerDribling.getValue(), 
							(Integer) spinnerPase.getValue(), 
							(Integer) spinnerVelocidad.getValue(),
							(Integer) spinnerDefensa.getValue(),
							(Integer) spinnerSalto.getValue(),
							myPlayer.getAficionado())) {
						
						user.restarPuntos(dif*10);
						
						myPlayer.setMedia((Integer) spinnerMedia.getValue());
						myPlayer.setTiro((Integer) spinnerTiro.getValue());
						myPlayer.setDribling((Integer) spinnerDribling.getValue());
						myPlayer.setPase((Integer) spinnerPase.getValue());
						myPlayer.setVelocidad((Integer) spinnerVelocidad.getValue());
						myPlayer.setDefensa((Integer) spinnerDefensa.getValue());
						myPlayer.setSalto((Integer) spinnerSalto.getValue());
						
						JOptionPane.showMessageDialog(null, "¡Mejora realizada!");
						...
						JOptionPane.showMessageDialog(null, "Hubo un error al mejorar, por favor volvé a intentarlo más tarde");
					}
				}
		...
		btnCancelar.setBounds(144, 421, 93, 32);
