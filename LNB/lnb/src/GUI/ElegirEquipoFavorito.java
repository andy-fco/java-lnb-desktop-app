...
public class ElegirEquipoFavorito extends JFrame {
	 ...
	 */
			...
			public void run(Aficionado user) {
				try {
					ElegirEquipoFavorito frame = new ElegirEquipoFavorito(user);
	 ...
	 * Create the frame.
	 */
	public ElegirEquipoFavorito(Aficionado user) {
		...
		textField.setBounds(72, 60, 459, 28);
		...
		scrollPane.setBounds(61, 109, 745, 249);
		...
		lblSeleccionado.setBounds(61, 368, 760, 20);
		...
		// Acción al seleccionar fila
				table.getSelectionModel().addListSelectionListener(e -> {
					if (!e.getValueIsAdjusting()) {
						...
						if (row != -1) {
							...
							lblSeleccionado.setText("Seleccionado: "+equipoSeleccionado.getNombre()+", "+equipoSeleccionado.getCiudad());
				...
				btnElegir.setBounds(656, 391, 150, 40);
				btnElegir.addActionListener(e -> {
					
					if (equipoSeleccionado == null) {
						lblSeleccionado.setText("Seleccione un equipo");
					} else {
						if (user.asignarEquipoFavorito(equipoSeleccionado)) {
							JOptionPane.showMessageDialog(null, "Equipo favorito asignado.");
							new Perfil(user).run(user);
							dispose();
						} else {
							JOptionPane.showMessageDialog(null, "Hubo un error al asignar equipo.\nPor favor vuelva a itnentarlo");
	...
	public void cargarTabla() {
	    ...
	    for (Equipo eq : equipos) {
	        model.addRow(new Object[] {
	            ...
	            eq.getCiudad(),
	            eq.getEstadio(),
	            eq.getFechaDeFundacion(), 
	            eq.getTemporadasEnLiga(),
	            eq.getCampeonatos()
	        });
	    }
	}
