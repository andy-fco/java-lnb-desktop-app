...
public class GestionUsers extends JFrame {
	...
	private JTextField txtBuscar_1;
	...
	public GestionUsers(Admin admin) {
		...
		txtBuscar.setBounds(11, 10, 459, 28);
		panel.add(txtBuscar);
		...
		model = new DefaultTableModel(new Object[][] {},
				new String[] { "ID", "Nombre", "Apellido", "Mail", "Contraseña", "Fecha de Nacimiento" }) {
		...
		model_1 = new DefaultTableModel(new Object[][] {},
				new String[] { "ID", "Nombre", "Apellido", "Contraseña" }) {
	...
	private void cargarTablaAdmin() {
		LinkedList<Admin> listados = DLLAdmin.mostrarAdmins().stream()
				.sorted(Comparator.comparing(Admin::getNombre, String.CASE_INSENSITIVE_ORDER))
				.collect(Collectors.toCollection(LinkedList::new));

		model_1.setRowCount(0);
		for (Admin u : listados) {
			model_1.addRow(new Object[] { u.getId(), u.getNombre(), u.getApellido(),  u.getPass()
					  });
		}
	}
	
	private void buscarAficionados(String texto) {
		LinkedList<Aficionado> listados = DLLAficionado.mostrarAficionados().stream()
				.filter(a -> a.getNombre().toLowerCase().contains(texto.toLowerCase())
						|| a.getApellido().toLowerCase().contains(texto.toLowerCase())
						|| a.getId().toLowerCase().contains(texto.toLowerCase()))
				.sorted(Comparator.comparing(Aficionado::getNombre, String.CASE_INSENSITIVE_ORDER))
				.collect(Collectors.toCollection(LinkedList::new));
	...
	private void buscarAdmins(String texto) {
		LinkedList<Admin> listados = DLLAdmin.mostrarAdmins().stream()
				.filter(a -> a.getNombre().toLowerCase().contains(texto.toLowerCase())
						|| a.getApellido().toLowerCase().contains(texto.toLowerCase())
						|| a.getId().toLowerCase().contains(texto.toLowerCase()))
